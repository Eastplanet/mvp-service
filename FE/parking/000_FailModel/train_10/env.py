...
"""
ver10
    시뮬레이션 상태 업그레이드 0.1초 변경
    정책 업그레이드 1초 변경
    max_steps = 600 60초로 변경
    max_speed = 5로 낮춤
    10-1
    성공률 0에 수렴, 시간 변경 및 최대 속도 제한
    10-2
    성공률 조금 상승, 속도를 더 낮추던가 어떤 부분이 문제인지 파악부터 해야겠음
    10-3
    도로크기 조정 
    10-4
    크기 배수로 100배로 키워서 설정
    10-5
    크기 배수로 5배 키워서 설정. 벽도 같이 키워야함
"""
...
class ParkingEnv(AbstractEnv, GoalEnv):
        ...
        config.update(
            {
                ...
                "policy_frequency": 1,      # 1초마다 에이전트의 행동 결정 
                "duration": 100,
                "screen_width": 800,
                "screen_height": 400,
    ...
    def _create_vehicles(self) -> None:
        ...
        for i in range(self.config["controlled_vehicles"]):
            ...
            # 차량 크기 설정
            vehicle.LENGTH = 15  # 길이 설정 (예: 4.5미터)
            vehicle.WIDTH = 28  # 너비 설정 (예: 2.0미터)
...
class ParkingEnvActionRepeat(ParkingEnv):
    def __init__(self):
        super().__init__({"policy_frequency": 1, "duration": 60})
