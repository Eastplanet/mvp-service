...
# 환경 생성 및 설정 로드
register(
    id='CustomParkingEnv_Kimura_v7',
    entry_point='env:ParkingEnv',  # entry_point 설정
    max_episode_steps=400, # 시뮬레이션 주파수와 에피소드 지속시간의 곱(duration)
)

# 환경 생성
env = gym.make("CustomParkingEnv_Kimura_v7")
...
# 10만번 학습(1e5~1e6)
model.learn(int(10000), callback=SuccessRateCallback())
